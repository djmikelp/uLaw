...
def compresor(x, u):
    ...
    i = 0
    ln1plus_u = np.log(1 + u)
    for data in x:
        temp = sgn(data) * (np.log(1 + (u * abs(data))) / ln1plus_u)
        y[i] = (temp)
        i = i + 1
    return y
...
archivoWav = "grabacion.wav"
archivoWav2 = "gcompress.wav"
...
print("*** normalizado ***")
for i in range(5):
    print(raw16BitsNormalizado[i], type(raw16Bits[i]))
